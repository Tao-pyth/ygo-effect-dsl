...
def test_current_production_distribution_gate_fails_closed_until_v1_evidence_exists() -> None:
    ...
    assert checks["security_threat_model_approved"]["passed"] is True
...
def test_production_distribution_gate_rejects_placeholder_pass_values(
    ...
    _write_json(
        evidence_dir / "v1_0_0_gated_release.json",
        {"passed": True, "schema_version": "v1-gated-release-v1"},
    )
    _write_json(
    ...
    _write_json(
        evidence_dir / "v1_0_0_security_threat_model.json",
        {"passed": True, "schema_version": "v1-security-threat-model-v1"},
    )
