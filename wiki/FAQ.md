...
A. 最小限の state/action semantics を engine boundary contract として導入します。たとえば `Deck`、`Hand`、`Field`、`GY`、`Banished`、`Extra` を読み、`draw`、`add_to_hand`、`send_to_gy`、`destroy`、`special_summon`、`banish`、`negate` などを後続 Bridge / Replay / Search / Evaluation へ渡す候補として扱います。Python が実ルールを実行するわけではありません。

## Q. Python は遊戯王ルールを実装しますか？

A. しません。ルール、合法手判定、状態遷移、Lua 実行の真実源は ocgcore / EDOPro Lua です。Python は DSL 変換、Replay、探索制御、評価、統計、実験制御を担当します。
