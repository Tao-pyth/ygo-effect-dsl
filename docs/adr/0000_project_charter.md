...
このリポジトリは、v0.0までカード効果テキストを構造化し、検証と分析を行う研究COREとして進んできた。一方で、長期的な目的はカードテキスト変換ではなく、遊戯王OCGの展開探索、妨害耐性解析、リカバリ解析、デッキ評価を自動化することである。現在のDSLはADR-0004で定義するRoute DSLである。
