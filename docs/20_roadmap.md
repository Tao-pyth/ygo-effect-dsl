...
Status: Package `0.8.0` Japanese-centered UI and i18n quality source release is current / `0.9.0` terminal preference and card selection UX is planned / `1.0.0` production and distribution gate is planned
...
今後の並びは `0.9.0` terminal preference and card selection UX、`1.0.0` production/distribution gate とする。Package version は配布物全体の SemVer、schema/contract version は保存形式と API の互換境界として独立管理する。
...
| Desktop terminal preference UX | package `0.9.0` planned | `card_code` を内部正本として維持しつつ、終端評価ルールの UI はデッキ別プロファイル、デッキ内カード選択、デッキ metadata、結果説明性を扱う |
...
| `0.9.0` | 終端評価ルール、カード選択、デッキ metadata、結果説明性 UX | [#324](https://github.com/Tao-pyth/ygo-effect-dsl/issues/324) | [v0.9.0](spec/v0.9.0/00_scope.md) | planned source milestone |
...
`0.9.0` は、終端プロファイルを利用者に伝わる終端評価ルールとして整理し、探索画面内編集を廃止して `[P] プロファイル` ページでデッキ別に管理する planned UX stage である。カード指定は検索入力ではなく選択中デッキ内カードから選ばせ、`card_code` は内部契約として維持し、効果テキストから合法性やルールを推測しない。あわせて、デッキ表示名/タグを metadata として扱い、結果画面では Help、停止理由、初期手札、最大スコア盤面を表示して、内部 hash を通常一覧の主表示から外す。
