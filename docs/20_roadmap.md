...
| Search | `search-executor-v3` / `search-frontier-v2` / `search-run-result-v3` / `random-search-strategy-v1` | state completenessとExperiment digest付きRandom Searchのみ実動。Beam/MCTSは未実装 |
| Real core | `real-core-frontier-v2` / `real-core-worker-failure-v1` | ocgcore API 11.0のfresh worker Replayとstate completeness |
