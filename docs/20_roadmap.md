...
| Search | `search-executor-v5` / `search-frontier-v2` / `search-run-result-v5` / `search-strategy-evidence-v1` / `search-run-report-v1` / `search-run-failure-v2` / `search-artifact-commit-v1` / `random-search-strategy-v1` / `beam-search-strategy-v1` | state completeness、structured failure、atomic commit付きRandom Searchと層単位Beam Searchが実動。MCTSは未実装 |
