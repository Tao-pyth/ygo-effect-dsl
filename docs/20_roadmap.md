...
2026-07-14に既存lock済みassetで10 nodeの`experiment-search`とbest Routeの`experiment-replay`を実行し、実ocgcore/EDOPro Luaのend-to-end動作を確認した。同日に空root、再実行、build/runtime partial、独立root、download partialの5 build clean-bootstrap qualificationも完了した。さらに外部short/long/grave-banishの3 deckをYDK/inlineとrandom/fixed/conditional handで各2反復し、sanitized evidence `deckqual_21dcdadd...`を取得した。任意deckの代表corpus gateは通過したが、未知Message形状、10万node実worker性能、競技デッキ全般の品質を保証しない。single-sessionのDLL hash一致をcross-host再現buildへ一般化せず、#171を維持する。
