...
production claimは上記4 fixture、固定core/API、固定script hash、観測済みmessage/payloadに限定する。任意カードの効果文からnegation種別やmissed timingを推測する機能、全`EVENT_*`、damage step、simultaneous trigger ordering、mandatory trigger、SEGOC、実カード固有裁定は未検証である。

`interruption-support-taxonomy-v1`は検証済みの`standard`と、未検証の4 categoryを機械可読に分離する。未検証categoryは明示的にpolicyへ登録しない限り`unsupported_category / unverified_fixture_category`で停止する。登録は試験専用の注入点であり、production対応を意味しない。

| Priority | Category | Current policy | Follow-up |
|---|---|---|---|
| 1 | damage step | fail-close | [#207](https://github.com/Tao-pyth/ygo-effect-dsl/issues/207) |
...
categoryを昇格するには、固定core/API/source/asset hash、fresh real-core Replay 2反復、Route/State lineage、DecisionRequest candidateとAction responseの証跡、改ざん検知validatorが必要である。Python側で効果、合法性、timing、orderingを推測してはならない。全`EVENT_*`と任意カード固有裁定はこの4 Issueを完了しても対象外であり、別のqualificationが必要となる。
