...
`.github/workflows/real-core-self-hosted-smoke.yml`は手動起動だけを許可し、`self-hosted/Windows/X64/ygo-real-core` labelとrunner-local `YGO_EFFECT_DSL_EXTERNAL_ROOT`を要求する。raw Lua、CDB、Route、deckはartifact化せず、lock、Route ID、terminal Stateを相互検証した`self-hosted-real-core-smoke-v1` summaryだけを14日保持する。2026-07-15の同一Windows host local command sequenceは`realcoresmoke_621138...`で成功し、ephemeral runner v2.335.1上の[workflow run 29380688844](https://github.com/Tao-pyth/ygo-effect-dsl/actions/runs/29380688844)も`realcoresmoke_8edbd574...`で成功した。sanitized summaryはartifact保持期限後も監査できるよう`docs/ocgcore/evidence/self_hosted_smoke_workflow.json`へ固定する。
