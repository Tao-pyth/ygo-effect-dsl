...
Windows desktop requirementは確定しているが、shell/frameworkは未確定である。[#181](https://github.com/Tao-pyth/ygo-effect-dsl/issues/181)でWebView2/pywebview、Tauri、Electron、Qt等を比較する。

選定時はPython integration、native worker ownership、installer size、startup、RSS、accessibility、Playwright、CSP/IPC、upgrade/rollbackを評価する。UI frameworkの都合でSearch/Replay contractやasset policyを変更しない。
...
| Decision | Owner |
|---|---|
| Windows shell、Python service、IPC、installer/update | [#181](https://github.com/Tao-pyth/ygo-effect-dsl/issues/181) |
