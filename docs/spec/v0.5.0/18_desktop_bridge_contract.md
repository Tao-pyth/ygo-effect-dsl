...
| `analytics.export.enqueue` | path-free versioned export job registration | forbidden |
...
The browser harness and pywebview shell both load `src/ygo_effect_dsl/desktop/static/index.html`. Browser execution retains the explicit synthetic search and 100,000-row analytics adapters. Desktop bridge availability enables the real local deck catalog, native YDK import, Python-owned Experiment composition, preflight, queue/status/cancel, continuously supervised background Search and export workers, `analytics.query`, and `analytics.export.enqueue`. The renderer applies the same `desktop-virtual-table-v1` cursor and DOM-window contract to both adapters, and never authors export bytes or accepts an output path. Persistent-storage scale calibration, signing, update, and distribution remain #167/#134/#91 work.

The same-host hidden WebView2 smoke loaded the packaged frontend and completed `system.describe`, an empty-snapshot `analytics.query`, `analytics.export.enqueue`, and export-worker artifact publication through `DesktopBridge.invoke`. The current evidence is `docs/ui/evidence/desktop_bridge_smoke.json`, ID `desktopbridgesmoke_000fd8c6e9b6af620d6cf6cd9cf0e2df80affeb1098a5870af275eea6ee3f333`. Lifecycle and performance measurements are kept separately so a browser/bridge pass cannot substitute for worker recovery evidence.
