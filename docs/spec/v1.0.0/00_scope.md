...
package `1.0.0`は、`0.3.0`から`0.5.0`でqualificationした機能を、明示的な互換性、license、build provenance、security、運用、support契約の下でstable releaseにするstageである。機能数ではなく、利用者がinstall、upgrade、rollback、監査、障害対応できることを完了条件にする。

## Entry criteria

- `0.5.0`までのcore/search/PlayerView/analytics gateが完了している。
