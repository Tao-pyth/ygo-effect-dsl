...
v0.0 の validation は「遊戯王の効果として完全に正しいか」ではなく、「DSL として後続処理が安全に読める形か」を確認する。新人プログラマーは、まず `error` と `warning` の違いを押さえると追いやすい。
...
| --- | --- | --- | --- |
| `error` | v0.0 DSL として必須構造を満たしていない。後続処理が安全に読めない。 | invalid | 1 |
...
v0.1 では、最小限の state/action semantics に進む予定である。たとえば `Deck` / `Hand` / `Field` / `GY` / `Banished` / `Extra` を読み、`draw` / `add_to_hand` / `send_to_gy` / `destroy` / `special_summon` / `banish` / `negate` のような action を実行可能な意味に近づける。

その前に v0.0 の diagnostics が必要な理由:

- `error` が残っていると、v0.1 の reader は DSL を安全に読めない。
- `unknown_action` が多いと、v0.1 の action executor が扱える語彙へ接続できない。
...
したがって v0.0 の validation diagnostics は、v0.1 の state/action semantics に進むための前提条件である。
