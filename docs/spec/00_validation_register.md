...
| `VAL-031` | deck-first desktop frontend | `desktop-workflow-v1`とpackaged local HTML/CSS/JSでdeck catalog、statistics、card detail、search/preflight、synthetic job/cancel/result、comparisonを実装。default-deny CSP、network/direct application-service accessなし、keyboard command、reduced motion/high contrast、Edge DOM 4 row、1440×900/960×700 screenshotを確認。100k virtualization、WebView2 lifecycle/a11yは#165/#245が未完了 | `VERIFIED_LOCAL` | [#182](https://github.com/Tao-pyth/ygo-effect-dsl/issues/182), [#184](https://github.com/Tao-pyth/ygo-effect-dsl/issues/184), [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165), [#245](https://github.com/Tao-pyth/ygo-effect-dsl/issues/245) |
| `VAL-032` | Windows desktop bridge | `desktop-bridge-v1`とpywebview 6.2.1/WebView2 shellを実装。単一`invoke` allowlist、request/response byte上限、renderer path拒否、native YDK import、content-addressed deck catalog、real asset preflight、SQLite search job queue/status/cancel、typed analytics/card adapter、single-instance lockを確認。同一packaged frontendからWebView2 live round-trip `desktopbridgesmoke_d018dd...`に成功。continuous worker、process-tree recovery、a11yは#245が未完了 | `VERIFIED_LOCAL` | [#244](https://github.com/Tao-pyth/ygo-effect-dsl/issues/244), [#245](https://github.com/Tao-pyth/ygo-effect-dsl/issues/245) |
