...
| `VAL-032` | Windows desktop bridge | `desktop-bridge-v1`とpywebview 6.2.1/WebView2 shellを実装。単一`invoke` allowlist、request/response byte上限、renderer path拒否、native YDK import、content-addressed deck catalog、Python側Experiment合成、real asset preflight、SQLite search job queue/status/cancel、typed analytics/card adapter、single-instance lockを確認。同一packaged frontendから`system.describe`と`analytics.query`のlive round-trip `desktopbridgesmoke_5062c060...`に成功 | `VERIFIED_LOCAL` | [#244](https://github.com/Tao-pyth/ygo-effect-dsl/issues/244), [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165) |
| `VAL-033` | Windows desktop lifecycle/recovery/a11y | `desktop-search-worker-v1`を非UI threadで実行し、Windows Job Objectの親子2 process終了後active 0、renderer/worker/host/timeout/cancelの独立failure、lease expiry後attempt 2とcheckpoint 17/100再開を確認。実ocgcore 10-node desktop jobは5 nodeで成功し、Route/Experiment/SearchRun/commit hash検証後に2 artifactをatomic commit。pywebview cold/warm、process family RSS、missing/outdated runtime、accessible name、focus、high contrast、reduced motion、CSP/path/method driftを`desktoplifecycleevidence_e294d061...`へ保存 | `VERIFIED_LOCAL` | [#245](https://github.com/Tao-pyth/ygo-effect-dsl/issues/245), [#134](https://github.com/Tao-pyth/ygo-effect-dsl/issues/134), [#91](https://github.com/Tao-pyth/ygo-effect-dsl/issues/91) |
| `VAL-034` | virtualized accessible analytics table | `desktop-virtual-table-v1`を`analytics.query`へ接続し、100,000行相当fixtureを500行cursorで読了。同時query 1、DOM最大28行、immutable snapshot、重複row拒否、sort/filter、column/density、keyboard/ARIA、1440×900/760×900非overlapを`desktopvirtualtableevidence_5983ee3c...`へ保存。実storage scaleは#167へ分離 | `VERIFIED_LOCAL` | [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
