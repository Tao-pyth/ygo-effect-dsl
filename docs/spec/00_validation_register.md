...
| `VAL-017` | analytics storage/UI | storage ADR、job recovery、query/comparison API、100k相当DOM virtualization、実storage scale校正を完了 | `VERIFIED_LOCAL` | [#133](https://github.com/Tao-pyth/ygo-effect-dsl/issues/133), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
...
| `VAL-021` | Windows desktop architecture | `windows-desktop-shell-selection-evidence-v1`でpywebview 6.2.1/WebView2をlive起動し、typed bridge、既存worker subprocess、非HTTP API、browser/desktop分離testを選定。Tauriはrollback候補、installer/update/signingは後続gate | `VERIFIED_LOCAL` | [#181](https://github.com/Tao-pyth/ygo-effect-dsl/issues/181) |
| `VAL-022` | card detail/effect text | `card-presentation-v1`をreal-core DataReaderから分離し、pinned BabelCDB hash/schema、read-only query、locale fallback、missing/redacted/stale/version diagnostic、Unicode/改行、level/rank/link/scale projection、Search/Replay非依存を検証。英語local sourceはsanitized evidenceで確認済み。非英語source/label driftは#247、再配布は#91が未完了 | `VERIFIED_LOCAL` | [#183](https://github.com/Tao-pyth/ygo-effect-dsl/issues/183), [#247](https://github.com/Tao-pyth/ygo-effect-dsl/issues/247), [#91](https://github.com/Tao-pyth/ygo-effect-dsl/issues/91) |
...
| `VAL-031` | deck-first desktop frontend | `desktop-workflow-v1`とpackaged local HTML/CSS/JSでdeck catalog、statistics、card detail、search/preflight、job/cancel/result、comparisonを実装。browser fixtureだけsynthetic adapterを維持し、desktopは実bridge jobを使用する。default-deny CSP、keyboard command、reduced motion/high contrast、Edge DOM 4 row、1440×900/960×700 screenshotを確認 | `VERIFIED_LOCAL` | [#182](https://github.com/Tao-pyth/ygo-effect-dsl/issues/182), [#184](https://github.com/Tao-pyth/ygo-effect-dsl/issues/184) |
| `VAL-032` | Windows desktop bridge | `desktop-bridge-v1`とpywebview 6.2.1/WebView2 shellを実装。単一`invoke` allowlist、request/response byte上限、renderer path拒否、native YDK import、content-addressed deck catalog、Python側Experiment合成、real asset preflight、SQLite search/export job queue/status/cancel、typed analytics/card adapter、single-instance lockを確認。同一packaged frontendから`system.describe`、`analytics.query`、`analytics.export.enqueue`とexport worker publishのlive round-trip `desktopbridgesmoke_000fd8c6...`に成功 | `VERIFIED_LOCAL` | [#244](https://github.com/Tao-pyth/ygo-effect-dsl/issues/244), [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165), [#166](https://github.com/Tao-pyth/ygo-effect-dsl/issues/166) |
| `VAL-033` | Windows desktop lifecycle/recovery/a11y | `desktop-search-worker-v1`を非UI threadで実行し、Windows Job Objectの親子2 process終了後active 0、renderer/worker/host/timeout/cancelの独立failure、lease expiry後attempt 2とcheckpoint 17/100再開を確認。実ocgcore 10-node desktop jobは5 nodeで成功し、Route/Experiment/SearchRun/commit hash検証後に2 artifactをatomic commit。pywebview cold/warm、process family RSS、missing/outdated runtime、accessible name、focus、high contrast、reduced motion、CSP/path/method driftを`desktoplifecycleevidence_307c69c7...`へ保存 | `VERIFIED_LOCAL` | [#245](https://github.com/Tao-pyth/ygo-effect-dsl/issues/245), [#134](https://github.com/Tao-pyth/ygo-effect-dsl/issues/134), [#91](https://github.com/Tao-pyth/ygo-effect-dsl/issues/91) |
| `VAL-034` | virtualized accessible analytics table | `desktop-virtual-table-v1`を`analytics.query`へ接続し、100,000行相当fixtureを500行cursorで読了。同時query 1、DOM最大28行、immutable snapshot、重複row拒否、sort/filter、column/density、keyboard/ARIA、1440×900/760×900非overlap、browser fixtureのbackend-owned export拒否を`desktopvirtualtableevidence_908bda8b...`へ保存。persistent storageは別workloadの`VAL-036`で検証 | `VERIFIED_LOCAL` | [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165), [#166](https://github.com/Tao-pyth/ygo-effect-dsl/issues/166), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
| `VAL-035` | versioned analytics export parity | `analytics-export-contract-v1`でsnapshot-bound queryと検証済みcomparisonをJSON/CSV/Parquetへ変換。三形式semantic digest、service/CLI subprocess/desktop bridge workerのdata・manifest bytes、formula/path/disk/row/byte制約、cancel/retry/atomic publishをpackage 0.5.0で再生成した`analyticsexportevidence_310ca486...`と自動testで確認。`VAL-036`の100,000行実測に基づきscan/output hard capを100,000行へ縮小 | `VERIFIED_LOCAL` | [#166](https://github.com/Tao-pyth/ygo-effect-dsl/issues/166), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167), [#255](https://github.com/Tao-pyth/ygo-effect-dsl/issues/255) |
| `VAL-036` | production analytics scale | `analytics-scale-calibration-v1`で10万run、100万Event/Decision row、10万aggregation rowをraw ingest、catalog、Parquet lifecycle、query/comparison、desktop bridge、三形式exportへ通した。crash/migration/rollback/concurrency/cancelを確認し、100,000行exportの最大peak RSS約2.48 GBを根拠にscan/output上限を100,000行へ縮小 | `VERIFIED_LOCAL` | [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
...
python -m ygo_effect_dsl.spikes.analytics_scale_calibration --runs 100000 --event-rows 1000000 --analytical-rows 100000 --out docs/storage/evidence/analytics_scale_calibration.json --samples-out docs/storage/evidence/analytics_scale_samples.parquet
```
