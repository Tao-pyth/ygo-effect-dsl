...
| `VAL-017` | analytics storage/UI | storage ADR、job recovery、query/comparison API、100k相当DOM virtualizationは完了。実storage 10万run/100万row校正は未完了 | `OPEN` | [#133](https://github.com/Tao-pyth/ygo-effect-dsl/issues/133), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
...
| `VAL-034` | virtualized accessible analytics table | `desktop-virtual-table-v1`を`analytics.query`へ接続し、100,000行相当fixtureを500行cursorで読了。同時query 1、DOM最大28行、immutable snapshot、重複row拒否、sort/filter、column/density、keyboard/ARIA、1440×900/760×900非overlap、browser fixtureのbackend-owned export拒否を`desktopvirtualtableevidence_908bda8b...`へ保存。実storage scaleは#167へ分離 | `VERIFIED_LOCAL` | [#165](https://github.com/Tao-pyth/ygo-effect-dsl/issues/165), [#166](https://github.com/Tao-pyth/ygo-effect-dsl/issues/166), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
| `VAL-035` | versioned analytics export parity | `analytics-export-contract-v1`でsnapshot-bound queryと検証済みcomparisonをJSON/CSV/Parquetへ変換。三形式semantic digest、service/CLI subprocess/desktop bridge workerのdata・manifest bytes、formula/path/disk/row/byte制約、cancel/retry/atomic publishを`analyticsexportevidence_babc372e...`と自動testで確認。production規模のmemory/latencyは#167へ分離 | `VERIFIED_LOCAL` | [#166](https://github.com/Tao-pyth/ygo-effect-dsl/issues/166), [#167](https://github.com/Tao-pyth/ygo-effect-dsl/issues/167) |
