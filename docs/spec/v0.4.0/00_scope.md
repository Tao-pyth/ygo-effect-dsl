...
Status: Implemented stage; superseded by cumulative package 0.5.0; node-level pool/cache is planned for package 0.6.0 in [#231](https://github.com/Tao-pyth/ygo-effect-dsl/issues/231)
...
この実装stageは2026-07-16に完了し、独立package tagを作らず累積package `0.5.0`へ収録した。damage step、simultaneous trigger、mandatory trigger、SEGOCは未検証時にfail-closeし、#207-#210で`1.0.0`のproduction qualificationとして継続する。node-level pool/cache統合#231はrelease完了時点で`1.0.0`へ分離した後、同日の計画変更により`0.6.0`の性能stageへ再割当した。
